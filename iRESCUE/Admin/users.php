...
if ($action === 'setrole' && isset($_GET['id']) && isset($_GET['role'])) {
    ...
    header('Location: ../Admin/users.php?toast=success&msg=Role+updated.'); exit;
}
...
if ($action === 'verify' && isset($_GET['id'])) {
    ...
    header('Location: ../Admin/users.php?toast=success&msg=User+verified.'); exit;
}
...
if ($action === 'delete' && isset($_GET['id'])) {
    ...
    header('Location: ../Admin/users.php?toast=success&msg=User+deleted.'); exit;
}
        ...
        <table id="users-table">
            ...
            <tr>
                ...
                <td>
                    ...
                    <?php else: ?>
                        <a href="../Admin/users.php?action=verify&id=<?php echo $u['id']; ?>" class="badge badge-pending" style="cursor:pointer;">✗ Unverified</a>
                    <?php endif; ?>
                </td>
                ...
                <td class="actions" style="display:flex;gap:6px;flex-wrap:wrap;">
                    ...
                    <select class="filter-select" style="font-size:.75rem;" onchange="location='../Admin/users.php?action=setrole&id=<?php echo $u['id']; ?>&role='+this.value">
                        <option value="">Set Role</option>
                        <option value="user">User</option>
                        <option value="responder">Responder</option>
                        <option value="admin">Admin</option>
                    </select>
                    <a href="../Admin/users.php?action=delete&id=<?php echo $u['id']; ?>" class="btn btn-sm btn-danger btn-delete">Delete</a>
                    <?php else: ?>
